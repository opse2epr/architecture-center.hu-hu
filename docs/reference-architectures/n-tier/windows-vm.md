...
Ez a cikk ismerteti a bevált gyakorlat az Azure-on futó Windows virtuális gépek (VM). Javaslatokat tartalmaz a VM, valamint a hálózatkezelési és tárolási összetevők üzembe helyezéséhez. [**A megoldás üzembe helyezése.**](#deploy-the-solution)
...
A virtuális gépeket a [virtuális gép méretének módosításával][vm-resize] skálázhatja. A horizontális felskálázáshoz helyezzen két vagy több virtuális gépet egy terheléselosztó mögé. További információkért lásd: a [N szintű referencia-architektúrában](./n-tier-sql-server.md).
