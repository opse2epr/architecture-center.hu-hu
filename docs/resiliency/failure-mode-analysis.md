...
**Helyreállítási**. Több [üzembe helyezési pontok] [ app-service-slots] , és állítsa vissza a legutóbbi megfelelően működő üzemelő példányhoz. További információkért lásd: [alapszintű webalkalmazás][ra-web-apps-basic].
...
<strong>Helyreállítási</strong>. Bírálja felül a [RoleEntryPoint.OnStop] [ RoleEntryPoint.OnStop] metódus szabályosan karbantartása. További információkért lásd: [Azure OnStop események kezeléséhez a jobb megoldást] [ onstop-events] (blog).
...
Előfeltétel: Az adatbázis az aktív georeplikáció kell konfigurálni. Lásd: [SQL adatbázis aktív Georeplikációt][sql-db-replication].
...
4. A további rugalmasság érdekében különböző régiókban lévő két a Service Bus-névtér létrehozása, és replikálja az üzeneteket. A replikálás aktív vagy passzív replikációs használhat.
   ...
   * Aktív replikáció: az ügyfél összes üzenetet küld mindkét üzenetsorok. A fogadó mindkét üzenetsorok figyel. Egy egyedi azonosítóval rendelkező üzenetek címkézése, így az ügyfél képes vesse el az ismétlődő üzeneteket.
   * Passzív replikáció: az ügyfél elküldi az üzenetet egy üzenetsorba. Ha hiba történik, az ügyfél visszavált az egyéb üzenetsorba. A fogadó mindkét üzenetsorok figyel. Ez a megközelítés csökkenti a duplikált küldött üzenetek számát. A fogadó azonban továbbra is kell kezelnie ismétlődő üzeneteket.
...
* Megszakítja a teljes alkalmazás hiba egész hibasorozatot indíthat ne kelljen hibák elhárítása érdekében. Kockázatcsökkentési stratégia a következők:
  ...
  * Alkalmazzon a [szabályozási minta] [ throttling-pattern] elsöprő háttérrendszerre elkerülése érdekében.
